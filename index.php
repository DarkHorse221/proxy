...
$landingExampleURL = "https://google.co.in";
...
if (empty($url)) {
    if (empty($startURL)) {
      die("<html><head><title>Proxy</title></head><body><h1>Proxy need a url to serve pages !</h1></body></html>");
    } else {
      $url = $startURL;
    }
...
if (stripos($contentType, "text/html") !== false) {
  ...
  if ($prependElem != null) {
    ...
    $httpsMeta=$doc->createElement("meta");
    $httpsMeta->setAttribute("http-equiv", "Content-Security-Policy");
    $httpsMeta->setAttribute("content", "upgrade-insecure-requests");    
    $prependElem->insertBefore($httpsMeta, $prependElem->firstChild);
  }

  if($body != null){
    $externalScript=$doc->createElement("script");
    $externalScript->setAttribute("type", "text/javascript");
    $externalScript->setAttribute("src", "./external.js");    
